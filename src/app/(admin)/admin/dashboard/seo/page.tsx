...
export default function SeoSettingsPage() {
  ...
  const enForm = useForm<SeoFormValues>({ resolver: zodResolver(seoSchema) });
  const arForm = useForm<SeoFormValues>({ resolver: zodResolver(seoSchema) });
  ...
  const handleSave = async () => {
    ...
    try {
        const payload = {
            en: enData,
            ar: arData,
            ogImageFile: ogImage.file
        };

        const formData = new FormData();
        formData.append('en', JSON.stringify(payload.en));
        formData.append('ar', JSON.stringify(payload.ar));
        if (payload.ogImageFile) {
            formData.append('ogImageFile', payload.ogImageFile);
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs defaultValue="en" className="w-full">
       ...
       <Card>
          <CardHeader>
            <CardTitle>Social Media Appearance (OpenGraph)</CardTitle>
            <CardDescription>
              This controls how your site looks when shared on platforms like Facebook, WhatsApp, etc. The image is shared across both languages.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                    <Form {...enForm}>
                        <FormField control={enForm.control} name="ogTitle" render={({ field }) => (
                          <FormItem><FormLabel>OpenGraph Title (English)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                        <FormField control={enForm.control} name="ogDescription" render={({ field }) => (
                          <FormItem><FormLabel>OpenGraph Description (English)</FormLabel><FormControl><Textarea rows={3} {...field} /></FormControl><FormMessage /></FormItem>
                        )} />
                    </Form>
                    <Form {...arForm}>
                         <div dir="rtl">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        ...
                        <label className="flex flex-col items-center justify-center w-full aspect-video border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted transition-colors max-h-24">
