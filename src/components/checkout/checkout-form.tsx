        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="grid md:grid-cols-3 gap-12">
                <div className="md:col-span-2 space-y-8">
                    <Card>
                        <CardHeader>
                            <CardTitle {...t('customer_information')}>{t('customer_information').text}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            <FormField control={form.control} name="fullName" render={({ field }) => (
                                <FormItem><FormLabel {...t('full_name')}>{t('full_name').text}</FormLabel><FormControl><Input placeholder="John Doe" {...field} /></FormControl><FormMessage /></FormItem>
                            )} />
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle {...t('shipping_address')}>{t('shipping_address').text}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {user && !areAddressesLoading && addresses && addresses.length > 0 && (
                                <FormItem>
                                    <FormLabel {...t('select_address')}>{t('select_address').text}</FormLabel>
                                    <Select onValueChange={handleAddressChange} value={selectedAddressId}>
                                        <FormControl>
                                        <SelectTrigger>
                                            <SelectValue placeholder={t('select_address').text} />
                                        </SelectTrigger>
                                        </FormControl>
                                        <SelectContent>
                                            {addresses.map(addr => (
                                     ...
                                     <FormDescription>
                                        <Link href="/account/addresses" className="text-sm text-primary hover:underline flex items-center gap-1">
                                            <PlusCircle className="h-4 w-4" /> {t('manage_saved_addresses').text}
                                        </Link>
                                    </FormDescription>
                                </FormItem>
                            )}
                     ...
                     <Card>
                        <CardHeader>
                            <CardTitle {...t('payment')}>{t('payment').text}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <FormField
                                ...
                                <div className="space-y-4 pt-4 border-t mt-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <FormField control={form.control} name="expiryDate" render={({ field }) => (
                                            <FormItem><FormLabel {...t('expiry_date')}>{t('expiry_date').text}</FormLabel><FormControl><Input placeholder="MM/YY" {...field} /></FormControl><FormMessage /></FormItem>
                                        )} />
